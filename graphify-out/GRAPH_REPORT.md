...
## Corpus Check
- 76 files · ~33,265 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 485 nodes · 895 edges · 28 communities (18 shown, 7 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1ea5b672`
...
2. `collection()` - 12 edges
3. `formatPercent()` - 12 edges
4. `PdiDoc` - 12 edges
...
Cohesion: 0.10
Nodes (39): Burndown(), pointOnPlanned(), addDays(), daysBetween(), num(), PLAN_BADGE, PlanningView(), ProgMode (+31 more)

### Community 2 - "index.ts"
Cohesion: 0.07
Nodes (40): GET(), runtime, Layout, groups, pdi, RFC-4180, seedHugo, resolveSeedGroups() (+32 more)
...
Cohesion: 0.19
Nodes (15): Home(), PdiCanvas(), EmptyState(), GeminiButton(), baseNodes(), swimlaneLayout(), br(), buildInsightPrompt() (+7 more)
...
Cohesion: 0.05
Nodes (50): POST(), runtime, isoDate, PATCH(), patchSchema, runtime, bodySchema, box (+42 more)

### Community 11 - "Checklist de Testes Manuais — PDI+ MVP (Rodadas 1–5)"
Cohesion: 0.05
Nodes (38): Prompt inicial — PDI+, Rodada 1 — Fundação e dados, Rodada 2 — Canvas read-only, Rodada 3 — Extensão Manifest V3, Rodada 4 — Drawer, edição manual, Gemini, modo gestor, PWA, ✅ Extra — Insight para 1-on-1 (Gemini), Progresso do PDI+, ⏭️ Próximas rodadas (+30 more)
...
## Knowledge Gaps
- **211 isolated node(s):** `nextConfig`, `name`, `version`, `private`, `dev` (+206 more)
...
- **Why does `react` connect `ManualBuilder.tsx` to `pdi-to-graph.ts`, `app/page.tsx`, `web/package.json`, `PlanningView.tsx`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `next-auth` connect `Auth Session Types` to `pdi-repo.ts`, `web/package.json`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `PdiDoc` connect `PlanningView.tsx` to `pdi-to-graph.ts`, `app/page.tsx`, `pdi-repo.ts`, `index.ts`?**
